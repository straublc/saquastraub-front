  ...
  <div class="p-4">
    ...
    <div class="card p-3 mb-3">
      ...
      <div class="row">
        <div class="col-md-4">
          <label>Nome</label>
          <input v-model="filtros.nome" type="text" class="form-control" placeholder="Digite o nome" @input="apenasLetras" />
        </div>
        <div class="col-md-4">
          <label>CPF</label>
          <input v-model="filtros.cpf" type="text" class="form-control" placeholder="Digite o CPF" @input="mascararCpf" maxlength="14" />
        </div>
    ...
    <div class="mb-3">
      ...
      <input type="text" v-model="valorFormatado" @input="formatarValor" class="form-control" required />
    </div>
    <div class="mb-3">
    ...
    <!-- Parcelado -->
    <div class="mb-3">
      <label>Parcelado <span class="text-danger">*</span></label>
      <select v-model="form.parcelado" class="form-select" required>
        <option value="">Selecione</option>
        <option value="sim">Sim</option>
        <option value="nao">Não</option>
      </select>
    </div>
    <div class="mb-3" v-if="form.parcelado === 'sim'">
    ...
    <button
      ...
      :disabled="!form.cliente_id || !form.data_inicio || !form.data_fim || !form.valor || !form.forma_pgto || !form.parcelado"
    >
      Cadastrar
    </button>
  </div>
...
// Força qtd_parcelas = 1 se não for parcelado
watch(
  () => form.value.parcelado,
  (novo) => {
    if (novo !== 'sim') {
      form.value.qtd_parcelas = 1
    }
  }
)
...
const cadastrarContrato = async () => {
  try {
    ...
    const res = await axios.post('http://localhost:3000/contratos', form.value, {
      headers: { Authorization: `Bearer ${localStorage.getItem('token')}` }
    })
